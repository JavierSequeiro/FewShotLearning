...
if __name__ == "__main__":
    ...
    logger.info(f"Working on: {args.experiment_name}")
        
    """
    Load Data
    """
    train_dataset, val_dataset, test_dataset = myProtoDataset(config=config,
    ...
    if "resnet" in config.model_name:
        model, arch_name = ResNet(config=config,
                                  logger=logger).load_pretrained_resnet_prototypes()
    ...
    if config.train_new_model: 
        trained_model, train_acc, val_acc, CI_err = myTrainerProto(config=config,
    ...
    Evaluator(model=trained_model,
              ...
              config=config,
              logger=logger).evaluate_proto_model()
